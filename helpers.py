...
db = sqlite3.connect('products.db')
cur = db.cursor()
cur.execute("""SELECT * FROM products WHERE category=? AND scrapedate=(SELECT MAX(scrapedate) FROM products WHERE category=?)""", ('mascara', 'mascara'))
product_rows = cur.fetchall()
db.close()
...
products.sort(key = lambda i: i['name'])
lf_names = [product['name'] for product in products if product['site_id'] == 'Look Fantastic']
hof_names = [product['name'] for product in products if product['site_id'] == 'House of Fraser']
print(hof_names)

for name in hof_names:
    match = process.extractOne(name, lf_names, scorer=fuzz.token_sort_ratio)
    print(match)